...
import StockCell4, {
  StockCell4Props,
} from "@/domains/wooribank/components/TikcerDisplay/StockCell4";
...
export default function WooriBankDisplay() {
  ...
  const itemPerRow = 34;
  const rowData: [StockCell4Props[], StockCell4Props[]] = useMemo(() => {
    const rows: [StockCell4Props[], StockCell4Props[]] = [[], []];
  ...
  return (
    <div
      className={`w-full h-[${screenHeight}px] min-h-[${screenHeight}px] max-h-[${screenHeight}px] bg-[#0d1a3b] overflow-hidden relative flex p-0`}
      style={{
            ...
            <div key={`row-${rowIndex}`} className="w-full relative">
              ...
              >
                {duplicated.map((stock, idx) => (
                  <StockCell4 key={`${rowIndex}-${idx}`} {...stock} />
                ))}
              </div>
            </div>
          );
